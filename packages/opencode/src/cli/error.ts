...
export function FormatError(input: unknown) {
  ...
  if (NamedError.hasName(input, "MCPFailed")) {
    return `MCP server "${(input as ErrorLike).data?.name}" failed. Note, BCS Code does not support MCP authentication yet.`
  }
  ...
  if (NamedError.hasName(input, "ProviderModelNotFoundError")) {
    ...
    return [
      ...
      `Try: \`bcs-code models\` to list available models`,
      `Or check your config (bcscode.json) provider/model names`,
    ].join("\n")
  }
