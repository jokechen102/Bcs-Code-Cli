    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (provider === "other") {
          ...
          prompts.log.warn(
            `This only stores a credential for ${provider} - you will need configure it in bcscode.json, check the docs for examples.`,
          )
        }
        ...
        if (provider === "amazon-bedrock") {
          prompts.log.info(
            "Amazon Bedrock authentication priority:\n" +
              ...
              "Configure via bcscode.json options (profile, region, endpoint) or\n" +
...
export const ProvidersWhoamiCommand = cmd({
  ...
  async handler(_args) {
    ...
    if (!info) {
      prompts.log.error("Not logged in. Run `bcs-code auth login` to log in.")
      return
    }
